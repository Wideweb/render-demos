...
void Framebuffer::addAttachment(const Texture& attachment, bool own) {
    switch (attachment.format) {
        ...
        default:
            ...
            glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + m_AttachmentsIndex, attachment.id, 0);
            Gfx::checkError();
    ...
    switch (attachment.format) {
        ...
        default:
            ...
            glFramebufferRenderbuffer(
                GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + m_AttachmentsIndex, GL_RENDERBUFFER, attachment.id
            );
            Gfx::checkError();
            ++m_AttachmentsIndex;
            std::vector<unsigned int> drawAttachments;
...
: m_Index(index),
  m_Type(Attachment::Type::Texture),
  m_Owned(owned) {
    id     = attachment.id;
...
: m_Index(index),
  m_Type(Attachment::Type::Renderbuffer),
  m_Owned(owned) {
    id     = attachment.id;
    format = attachment.format;
}
...
void Framebuffer::Attachment::read(int x, int y, int width, int height, void* buffer) {
    ...
    glReadPixels(
        x, y, width, height, GfxImage::getNativeDataFormat(GfxImage::formatToDataFormat(format)),
